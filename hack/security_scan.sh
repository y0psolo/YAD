...
REPO="yadist"
JAVA_VERSIONS=$(grep 'JRE_VERSIONS =' update/constant.bzl | grep -o -E "[0-9]+")
NODEJS_VERSIONS=$(grep 'NODEJS_VERSIONS =' update/constant.bzl | grep -o -E "[0-9]+")
DENO_VERSION=$(grep 'DENO_VERSION =' update/constant.bzl | grep -o -m 1 -E "[0-9]+\.[0-9]+\.[0-9]+")
TRINO_VERSION=$(grep TRINO_VERSION update/constant.bzl | grep -o -m 1 -E "[0-9]+")
HIVE_VERSION=$(grep HIVE_VERSION update/constant.bzl | grep -o -m 1 -E "[0-9].[0-9].[0-9]")
SPARK_VERSIONS=$(grep SPARK_VERSIONS update/constant.bzl | grep -o -E "[0-9].[0-9]")
